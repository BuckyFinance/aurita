module account::exit_positions_manager {
    ...
    public fun withdraw_logic<CoinType>(
        ...
        if(is_withdraw_allowed) {
            unsafe_withdraw_logic<CoinType>(sender, to_withdraw, receiver, max_iterations_for_matching);
        };
    }
    ...
    public fun repay_logic<CoinType>(
        ...
        unsafe_repay_logic<CoinType>(sender, on_behalf, to_repay, max_iterations_for_matching);
    }
    ...
    fun unsafe_withdraw_logic<CoinType>(
        ...
        if(remaining_to_withdraw > 0 && head_supplier_on_pool != @0x0) {
            let (matched, iteration_consumed_in_matching) = matching_engine::match_supplier<CoinType>(sender, remaining_to_withdraw, remaining_iterations_for_matching); 
            if(remaining_iterations_for_matching <= iteration_consumed_in_matching) {
                remaining_iterations_for_matching = 0;
            } else {
                remaining_iterations_for_matching = remaining_iterations_for_matching - iteration_consumed_in_matching;
            };
        ...
        if(remaining_to_withdraw > 0) {
            let (unmatched, iterations) = matching_engine::unmatch_borrower<CoinType>(sender, remaining_to_withdraw, remaining_iterations_for_matching); 
    ...
    fun unsafe_repay_logic<CoinType>(
        ...
        let (pool_supply_index, pool_borrow_index) = storage::get_pool_index<CoinType>();
        let (borrower_balance_in_p2p, borrower_balance_on_pool) = storage::get_borrow_balance<CoinType>(sender_addr);
        let to_repay = 0;
        ...
        if(borrower_balance_on_pool > 0) {
            ...
            borrower_balance_on_pool = borrower_balance_on_pool - math::min(
        ...
        let (p2p_supply_delta, p2p_borrow_delta, p2p_supply_amount, p2p_borrow_amount) = storage::get_delta<CoinType>();
        let (p2p_supply_index, p2p_borrow_index) = storage::get_p2p_index<CoinType>();
        borrower_balance_in_p2p = borrower_balance_in_p2p - math::min(
            borrower_balance_in_p2p,
            math::ray_div(remaining_to_repay, p2p_borrow_index),
        );

        storage::update_borrow_record<CoinType>(sender_addr, borrower_balance_in_p2p, borrower_balance_on_pool);
        ...
        if(remaining_to_repay > 0 && p2p_borrow_delta > 0) {
            ...
            p2p_borrow_delta = p2p_borrow_delta - math::min(
                p2p_borrow_delta,
                math::ray_div(remaining_to_repay, pool_borrow_index)
            );

            p2p_borrow_amount = p2p_borrow_amount - math::ray_div(matched_delta, p2p_borrow_index);
        ...
        if(remaining_to_repay > 0) {
            ...
            let fee_supply = math::ray_mul(p2p_supply_amount, p2p_supply_index) - math::min(
            ...
            if(fee_to_repay > 0 ) {
                ...
                p2p_borrow_amount = p2p_borrow_amount - math::ray_div(
                    fee_repaid,
                    p2p_borrow_index
                );
            }
        ...
        if(remaining_to_repay > 0 && head_borrower_on_pool != @0x0) {
            let (matched, iteration_consumed_in_matching) = matching_engine::match_supplier<CoinType>(sender, remaining_to_repay, remaining_iterations_for_matching); 
            if(remaining_iterations_for_matching <= iteration_consumed_in_matching) {
                remaining_iterations_for_matching = 0;
            } else {
                remaining_iterations_for_matching = remaining_iterations_for_matching - iteration_consumed_in_matching;
        ...
        // breaking withdraw -> demote borrower
        if(remaining_to_repay > 0) {
            let (unmatched, iterations) = matching_engine::unmatch_supplier<CoinType>(sender, remaining_to_repay, remaining_iterations_for_matching); 
            if(unmatched < remaining_to_repay) {
                p2p_supply_delta = p2p_supply_delta + math::ray_div(remaining_to_repay - unmatched, pool_supply_index);
            };
