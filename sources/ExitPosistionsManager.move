    ...
    fun unsafe_withdraw_logic<CoinType>(
        ...
        if(on_pool_supply > 0) {
            ...
            if(remaining_to_withdraw == 0) {
                ...
                // withdraw from pool
                let coin = PosUtils::withdraw<CoinType>(sender, to_withdraw); // @todo: sender must be this contract
        ...
        if(to_withdraw > 0) {
            let withdraw_coin = PosUtils::withdraw<CoinType>(sender, to_withdraw);
        ...
        if(remaining_to_withdraw > 0) {
            ...
            let borrow_coin = PosUtils::borrow<CoinType>(sender, remaining_to_withdraw);
