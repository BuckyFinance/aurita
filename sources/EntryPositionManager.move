module account::entry_positions_manager {
    ...
    public entry fun supply<CoinType>(user: &signer, on_behalf: address, amount: u256, iterations: u256) {
    ...
    public entry fun borrow<CoinType>(user: &signer, amount: u256, iterations: u256) {
        let user_addr = signer::address_of(user);
        assert!(amount > 0, EAMOUNT_ZERO);
        ...
        if (p2p_supply_delta > 0) {
            let matched_delta = math::min(vars.remain_to_borrow, math::ray_mul(p2p_supply_delta, vars.pool_supply_index));
            p2p_supply_delta = p2p_supply_delta - math::ray_div(matched_delta, vars.pool_supply_index);
        ...
        if (vars.remain_to_borrow > 0) {
            let (matched_amount, remain_iterations) = matching_engine::match_supplier<CoinType>(user, vars.remain_to_borrow, iterations);
            vars.to_withdraw = vars.to_withdraw + matched_amount;
            vars.remain_to_borrow = vars.remain_to_borrow - matched_amount;
            p2p_supply_amount = p2p_supply_amount + math::ray_div(matched_amount, p2p_supply_index);
        };
        
        let (user_borrow_in_p2p, user_borrow_on_pool) = storage::get_borrow_balance<CoinType>(user_addr);
        ...
        if (vars.to_withdraw > 0) {
            ...
            coin::merge(&mut withdraw_coin, pos_utils::withdraw<CoinType>(user, vars.to_withdraw));
        };

        // borrow remaining from pool
        if (vars.remain_to_borrow > 0) {
            user_borrow_on_pool = user_borrow_on_pool + math::ray_div(vars.remain_to_borrow, pool_borrow_index);
            coin::merge(&mut withdraw_coin, pos_utils::borrow<CoinType>(user, vars.remain_to_borrow));
        };


        // update user borrow balance
        storage::update_borrow_record<CoinType>(user_addr, user_borrow_in_p2p, user_borrow_on_pool);
        // update delta
        storage::set_delta<CoinType>(p2p_supply_delta, p2p_borrow_delta, p2p_supply_amount, p2p_borrow_amount);
        // update in heap
