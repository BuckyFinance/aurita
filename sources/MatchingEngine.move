module account::matching_engine {
    ...
    */
    ...
    public fun match_borrower<CoinType>(user: &signer, amount: u256, max_iteration: u256) : u256 {
        ...
        let matched_borrower = storage::get_head_borrower_in_p2p<CoinType>();
    ...
    public fun update_supplier_in_DS<CoinType>(user: address, amount: u256) {
        storage::remove_supplier_in_p2p<CoinType>(user);
        storage::add_supplier_in_p2p<CoinType>(user);
    }

    // refactor later
    public fun update_borrower_in_DS<CoinType>(user: address, amount: u256) {
        storage::remove_borrower_in_p2p<CoinType>(user);
        storage::add_borrower_in_p2p<CoinType>(user);
    }
}